 ...
 * @description Logic for the Installation Settings tab.
 */
...
function setupApiGuideToggles() {
    // Set up API guide toggle functionality
    document.querySelectorAll('.api-guide-toggle').forEach(toggle => {
        ...
        Object.entries(settings.customParameters).forEach(([name, value]) => {
            ...
            paramElement.innerHTML = `
                <div class="custom-param-row">
                    ...
                    <input type="text" placeholder="Description" class="param-desc" value="" />
...
async function testInstallationSettings() {
    try {
        ...
        showLoading('Testing installation settings...');
        ...
        if (response.success) {
            ...
            showTestResultsModal(response.results || {});
        } else {
            ...
            <div class="modal-body">
                <div class="test-results">
                    <div class="test-section">
                        <h4><i class="fas fa-camera"></i> Camera Settings</h4>
                        <div class="test-item ${results.camera?.status || 'pending'}">
                            <span class="test-label">Threshold validation:</span>
                            <span class="test-status">${results.camera?.threshold || 'Pending'}</span>
                        </div>
                        <div class="test-item ${results.camera?.status || 'pending'}">
                            <span class="test-label">Timeout validation:</span>
                            <span class="test-status">${results.camera?.timeout || 'Pending'}</span>
                        </div>
                    </div>
                    
                    <div class="test-section">
                        <h4><i class="fas fa-hand-paper"></i> Capacitive Settings</h4>
                        <div class="test-item ${results.capacitive?.status || 'pending'}">
                            <span class="test-label">Threshold validation:</span>
                            <span class="test-status">${results.capacitive?.threshold || 'Pending'}</span>
                        </div>
                        <div class="test-item ${results.capacitive?.status || 'pending'}">
                            <span class="test-label">Pin configuration:</span>
                            <span class="test-status">${results.capacitive?.pins || 'Pending'}</span>
                        </div>
                    </div>
                    
                    <div class="test-section">
                        <h4><i class="fas fa-volume-up"></i> Audio Settings</h4>
                        <div class="test-item ${results.audio?.status || 'pending'}">
                            <span class="test-label">Threshold validation:</span>
                            <span class="test-status">${results.audio?.threshold || 'Pending'}</span>
                        </div>
                        <div class="test-item ${results.audio?.status || 'pending'}">
                            <span class="test-label">Sample rate:</span>
                            <span class="test-status">${results.audio?.sampleRate || 'Pending'}</span>
                        </div>
                    </div>
                    
                    <div class="test-section">
                        <h4><i class="fas fa-wifi"></i> Network Settings</h4>
                        <div class="test-item ${results.network?.status || 'pending'}">
                            <span class="test-label">Connectivity test:</span>
                            <span class="test-status">${results.network?.connectivity || 'Pending'}</span>
                        </div>
                        <div class="test-item ${results.network?.status || 'pending'}">
                            <span class="test-label">Timeout validation:</span>
                            <span class="test-status">${results.network?.timeout || 'Pending'}</span>
                        </div>
                    </div>
                </div>
            </div>
...
async function applyInstallationSettings() {
    try {
        const settings = getCurrentSettings();
        showLoading('Applying installation settings...');
        ...
        await saveInstallationMasterConfig();
...
function addCustomParameter(name = '', value = '', description = '') {
    const customParamsList = document.getElementById('custom-params-list');
    ...
    paramElement.innerHTML = `
        <div class="custom-param-row">
            <input type="text" placeholder="Parameter name" class="param-name" value="${name}" />
            <input type="text" placeholder="Parameter value" class="param-value" value="${value}" />
            <input type="text" placeholder="Description" class="param-desc" value="${description}" />
            <select class="param-type">
                <option value="string">String</option>
                <option value="number">Number</option>
                <option value="boolean">Boolean</option>
...
export { initInstallationSettings };