...
class BackendService {
  constructor() {
    ...
    console.log(
      `🚀 ${SERVICE_CONFIG.name} v${SERVICE_CONFIG.version} starting in ${SERVICE_CONFIG.mode} mode...`
    );
    ...
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(SERVICE_CONFIG.port, '0.0.0.0', error => {
        if (error) {
          ...
          this.log(
            `Web interface: ${SERVICE_CONFIG.enableWebInterface ? 'enabled' : 'disabled'}`
          );
          resolve();
        }
      });
  ...
  startMonitoring() {
    try {
      ...
      setInterval(() => {
        ...
        this.log(
          `Status: Running ${uptime}s, Memory: ${Math.round(memUsage.rss / 1024 / 1024)}MB`,
          'STATUS'
        );
