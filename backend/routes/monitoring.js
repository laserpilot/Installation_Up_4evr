...
module.exports = platformManager => {
  ...
  router.get('/applications', async (req, res) => {
    try {
      ...
      if (result.success) {
        ...
        res
          .status(500)
          .json({ error: result.error || 'Failed to get applications data' });
      }
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  ...
  router.put('/ping-monitors/:id', async (req, res) => {
    try {
      const result = await platformManager.handleAPIRequest(
        `/monitoring/ping-monitors/${req.params.id}`,
        'PUT',
        req.body,
        { params: req.params }
      );
  ...
  router.post('/ping-monitors/:id/test', async (req, res) => {
    try {
      const result = await platformManager.handleAPIRequest(
        `/monitoring/ping-monitors/${req.params.id}/test`,
        'POST',
        req.body,
        { params: req.params }
      );
