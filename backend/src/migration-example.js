...
class ModernizedServer {
  ...
  setupRoutes() {
    ...
    this.app.use('/api', async (req, res, next) => {
      try {
        ...
        const result = await this.platform.handleAPIRequest(
          path,
          method,
          data,
          context
  ...
  async start() {
    ...
    this.app.listen(this.port, () => {
      console.log(
        `🚀 Modernized Installation Up 4evr server running on http://localhost:${this.port}`
      );
      console.log(`Frontend available at: http://localhost:${this.port}`);
      console.log(
        `API endpoints available at: http://localhost:${this.port}/api/*`
      );
