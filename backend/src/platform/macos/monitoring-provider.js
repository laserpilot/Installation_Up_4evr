...
class MacOSMonitoringProvider {
  ...
  async getTopMemoryProcesses(limit = 5) {
    try {
      ...
      const { stdout } = await execAsync(
        'ps -axo pid,comm,%mem,rss -m | head -n ' + (limit + 1)
      );
  ...
  async getTopCPUProcesses(limit = 5) {
    try {
      ...
      const { stdout } = await execAsync(
        'ps -axo pid,comm,%cpu -r | head -n ' + (limit + 1)
      );
      const lines = stdout.trim().split('\n').slice(1); // Skip header
  ...
  async getDiskUsage() {
    try {
      ...
      let mainVolume = volumes.find(
        v => v.mountPoint === '/System/Volumes/Data'
      );
    ...
    try {
      ...
      if (data.SPDisplaysDataType) {
        data.SPDisplaysDataType.forEach((graphicsCard, cardIndex) => {
          if (
            graphicsCard.spdisplays_ndrvs &&
            Array.isArray(graphicsCard.spdisplays_ndrvs)
          ) {
