...
class MacOSSystemManager extends SystemManagerInterface {
  ...
  initializeSettings() {
    return {
      ...
      doNotDisturb: {
        ...
        description:
          'Enable Do Not Disturb from midnight to midnight (prevents notification interruptions)',
        command:
          'plutil -replace dndStart -integer 0 ~/Library/Preferences/com.apple.ncprefs.plist && plutil -replace dndEnd -integer 1440 ~/Library/Preferences/com.apple.ncprefs.plist && defaults write com.apple.ncprefs doNotDisturb -bool true',
        revert:
          'defaults delete com.apple.ncprefs doNotDisturb 2>/dev/null || true',
        verify:
          'defaults read com.apple.ncprefs doNotDisturb 2>/dev/null || echo "0"',
        required: false,
        category: 'ui'
      },
      hideMenuBar: {
      ...
      hideDesktopIcons: {
        ...
        verify:
          'defaults read com.apple.finder CreateDesktop 2>/dev/null || echo "1"',
        required: false,
        category: 'ui'
      },
      autohideDock: {
        name: 'Auto-hide Dock',
        description:
          'Automatically show and hide Dock (recommended for installations)',
        command:
      ...
      disableBluetoothSetup: {
        ...
        command:
          'sudo defaults write /Library/Preferences/com.apple.Bluetooth BluetoothAutoSeekPointingDevice -bool false && sudo defaults write /Library/Preferences/com.apple.Bluetooth BluetoothAutoSeekKeyboard -bool false',
        revert:
          'sudo defaults delete /Library/Preferences/com.apple.Bluetooth BluetoothAutoSeekPointingDevice 2>/dev/null || true && sudo defaults delete /Library/Preferences/com.apple.Bluetooth BluetoothAutoSeekKeyboard 2>/dev/null || true',
        verify:
          'defaults read /Library/Preferences/com.apple.Bluetooth BluetoothAutoSeekPointingDevice 2>/dev/null || echo "1"',
        required: false,
        category: 'system'
      },
      disableAppNap: {
      ...
      doNotDisturb: {
        ...
        verify:
          'defaults -currentHost read ~/Library/Preferences/ByHost/com.apple.notificationcenterui doNotDisturb 2>/dev/null || echo "false"',
        required: false,
        category: 'ui'
      },
      hideDesktopIcons: {
      ...
      disableStageManager: {
        ...
        description:
          'Disable Stage Manager (macOS Ventura+) to prevent windowing interference',
        command:
        ...
        verify:
          'defaults read com.apple.WindowManager GloballyEnabled 2>/dev/null || echo "false"',
        required: false,
        category: 'ui'
      },
      disableNetworkPrompts: {
        ...
        command:
          'sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.airport JoinMode -string "Automatic"',
        revert:
          'sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.airport JoinMode -string "Prompt"',
        verify:
          'defaults read /Library/Preferences/SystemConfiguration/com.apple.airport JoinMode 2>/dev/null || echo "Prompt"',
        required: false,
        category: 'network'
      },
      disableGatekeeper: {
        ...
        description:
          '⚠️ SECURITY RISK: Allow apps from unidentified developers',
      ...
      allowAppsAnywhere: {
        ...
        description:
          '⚠️ SECURITY RISK: Allow apps from anywhere (Gatekeeper bypass)',
        command: 'sudo spctl --master-disable',
        revert: 'sudo spctl --master-enable',
        verify: 'spctl --status',
        required: false,
      ...
      disableCrashReporter: {
        ...
        description:
          "⚠️ DANGER: Prevent 'Application Unexpectedly Quit' dialogs (requires SIP disabled)",
        command:
          'sudo chmod 000 "/System/Library/CoreServices/Problem Reporter.app"',
        revert:
          'sudo chmod 755 "/System/Library/CoreServices/Problem Reporter.app"',
        verify:
          'ls -ld "/System/Library/CoreServices/Problem Reporter.app" | awk \'{print $1}\'',
        required: false,
        category: 'danger'
      },
      enableAutomaticLogin: {
        ...
        description:
          '⚠️ SECURITY RISK: Enable automatic login for scheduled reboots (removes login security)',
        command:
          'sudo defaults write /Library/Preferences/com.apple.loginwindow autoLoginUser -string "$(whoami)"',
        revert:
          'sudo defaults delete /Library/Preferences/com.apple.loginwindow autoLoginUser',
        verify:
          'defaults read /Library/Preferences/com.apple.loginwindow autoLoginUser 2>/dev/null || echo "Not set"',
        required: false,
        category: 'danger'
      }
    };
  ...
  async getSystemInfo() {
    try {
      const { stdout: systemVersion } = await execAsync(
        'sw_vers -productVersion'
      );
      const { stdout: buildVersion } = await execAsync('sw_vers -buildVersion');
      const { stdout: computerName } = await execAsync(
    ...
    for (const key of settingKeys) {
      ...
      if (this.logger) {
        await this.logger.system(
          1,
          `Applying system setting: ${setting.name}`,
          {
            settingKey: key,
      ...
      try {
        ...
        if (setting.command.startsWith('sudo ')) {
          ...
          const result = await this.executeSudoCommand(
            commandWithoutSudo,
            setting.name
          );
          ...
          if (this.logger) {
            await this.logger.system(
              1,
              `System setting applied successfully: ${setting.name}`,
              {
                settingKey: key,
                settingName: setting.name,
                success: true,
        ...
        if (this.logger) {
          await this.logger.system(
            3,
            `System setting failed: ${setting.name}`,
            {
              settingKey: key,
              settingName: setting.name,
              success: false,
...
`;
  ...
  evaluateSettingStatus(settingKey, output) {
    ...
    switch (settingKey) {
      ...
      case 'enableAutomaticLogin':
        ...
        return {
          applied: !output.includes('Not set') && output.trim().length > 0
        };
  ...
  async generateCommands(selectedSettings = null) {
    ...
    commands.push(
      'echo "Installing Up 4evr - System Preferences Configuration"'
    );
    commands.push(
      `echo "Applying ${settingsToInclude.length} selected setting${settingsToInclude.length !== 1 ? 's' : ''}..."`
    );
    commands.push('echo "==========================================="');
    commands.push('');
    ...
    settingsToInclude.forEach((key, index) => {
      const setting = this.settings[key];
      commands.push(
        `echo "[${index + 1}/${settingsToInclude.length}] Configuring ${setting.name}..."`
      );
    ...
    commands.push(
      `echo "Successfully applied ${settingsToInclude.length} setting${settingsToInclude.length !== 1 ? 's' : ''}"`
    );
    commands.push(
  ...
  async generateRestore() {
    ...
    commands.push('');
    commands.push(
      'echo "⚠️  WARNING: This will restore all system settings to defaults"'
    );
    ...
    return {
      ...
      data: {
        commands: commands.join('\n'),
        count: Object.keys(this.settings).filter(s => this.settings[s].revert)
          .length,
        timestamp: new Date().toISOString()
      }
    };
