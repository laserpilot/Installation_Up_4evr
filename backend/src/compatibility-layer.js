...
class CompatibilityLayer {
  ...
  async initialize(usePlatformManager = true) {
    ...
    if (!this.legacyMode) {
      try {
        ...
        console.warn(
          '[COMPATIBILITY] Platform manager failed, falling back to legacy mode:',
          error.message
        );
        this.legacyMode = true;
        this.platformManager = null;
    ...
    console.log(
      `[COMPATIBILITY] Initialized in ${this.legacyMode ? 'legacy' : 'platform'} mode`
    );
  }
  ...
  async getSystemPrefsStatus(legacySystemPrefs) {
    if (this.isUsingPlatformManager()) {
      const result = await this.platformManager.handleAPIRequest(
  ...
  async getConfiguration() {
    if (this.isUsingPlatformManager()) {
      ...
      return {
        message: 'Configuration management requires platform manager mode'
      };
    }
  }
  ...
  async updateConfiguration(path, value) {
    if (this.isUsingPlatformManager()) {
      ...
      throw new Error(
        'Configuration management requires platform manager mode'
      );
    }
  }
      ...
      try {
        if (this.isUsingPlatformManager()) {
          ...
          try {
            ...
            console.warn(
              `[COMPATIBILITY] Platform manager failed for ${method} ${path}, falling back to legacy:`,
