...
class HealthScoringEngine {
  constructor() {
    this.weights = {
      ...
      security: 0.2, // Updates, security settings
      configuration: 0.2 // Optimal settings, completeness
    };
  ...
  calculatePerformanceScore(systemData) {
    ...
    if (cpu?.usage !== undefined) {
      const cpuScore = this.calculateResourceScore(cpu.usage, {
        good: 70,
        fair: 85,
        poor: 95
      });
    ...
    if (memory?.usage !== undefined) {
      const memoryScore = this.calculateResourceScore(memory.usage, {
        good: 75,
        fair: 85,
        poor: 95
      });
    ...
    if (disk?.usage !== undefined) {
      const diskScore = this.calculateResourceScore(disk.usage, {
        good: 80,
        fair: 90,
        poor: 95
      });
    ...
    if (temperature?.cpu !== undefined) {
      const tempScore = this.calculateResourceScore(temperature.cpu, {
        good: 70,
        fair: 80,
        poor: 90
      });
  ...
  calculateConfigurationScore(configData) {
    ...
    const monitoringScore = this.evaluateMonitoringConfig(
    ...
    const notificationScore = this.evaluateNotificationConfig(
    ...
    const installationScore = this.evaluateInstallationConfig(
      configData.installation
    );
  ...
  calculateResourceScore(usage, thresholds) {
    ...
    if (usage <= thresholds.fair)
      return (
        80 -
        ((usage - thresholds.good) / (thresholds.fair - thresholds.good)) * 20
      );
    if (usage <= thresholds.poor)
      return (
        60 -
        ((usage - thresholds.fair) / (thresholds.poor - thresholds.fair)) * 20
      );
    return Math.max(
      0,
      40 - ((usage - thresholds.poor) / (100 - thresholds.poor)) * 40
    );
  }
  ...
  calculateApplicationScore(applications) {
    ...
    const shouldBeRunning = applications.filter(
      app => app.shouldBeRunning
    ).length;
  ...
  calculateDisplayScore(displays) {
    ...
    const onlineDisplays = displayArray.filter(
  ...
  evaluateNotificationConfig(notifications = {}) {
    ...
    if (notifications.channels) {
      const enabledChannels = Object.values(notifications.channels).filter(
        ch => ch.enabled
      ).length;
          ...
          case 'uptime':
            if (factor.hours < 24) {
              recommendations.push({
                ...
                action:
                  'Check system logs for crash reports or unexpected restarts',
                impact: 'Stability'
              });
            }
            break;
          case 'applications':
            const stoppedApps = factor.apps.filter(
              app => app.shouldBeRunning && app.status !== 'running'
            );
            if (stoppedApps.length > 0) {
              recommendations.push({
                ...
                action:
                  'Restart stopped applications and check their configuration',
                impact: 'Installation Functionality'
              });
            }
            break;
          case 'displays':
            const offlineDisplays = Object.values(factor.displays).filter(
              d => !d.online
            );
        ...
        switch (factor.type) {
          case 'sip':
            recommendations.push({
              ...
              action:
                'Enable SIP unless specifically required for your installation',
              impact: 'Security'
            });
            break;
          case 'gatekeeper':
            recommendations.push({
              ...
              description:
                'Gatekeeper is disabled, allowing unsigned apps to run.',
          ...
          case 'firewall':
            recommendations.push({
              ...
              description:
                'System firewall is disabled, reducing network security.',
  ...
  generateHealthReport(systemData, configData = {}) {
    ...
    return {
      ...
      summary: {
        criticalIssues: healthScore.recommendations.filter(
          r => r.priority === 'critical'
        ).length,
        highPriorityIssues: healthScore.recommendations.filter(
          r => r.priority === 'high'
        ).length,
