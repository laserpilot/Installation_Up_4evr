...
class PlatformManager {
  ...
  async initialize() {
    ...
    try {
      ...
      this.logger = new Logger({
        logLevel: config.monitoring?.debugMode
          ? LOG_LEVELS.DEBUG
          : LOG_LEVELS.INFO,
      ...
      this.validation = new ValidationWorkflow(
        this.config,
        this.monitoring,
        this.systemManager
      );
  ...
  setupAPIRoutes() {
        ...
        await this.logger.security(
          ...
          `System settings application ${result.success ? 'completed successfully' : 'completed with failures'}`,
          {
            ...
            action: result.success
              ? 'system_settings_apply_success'
              : 'system_settings_apply_partial_failure'
          }
        );
      }
    ...
    this.api.registerRoute(
      ...
      async () => {
        ...
        const requiredSettings = Object.keys(allSettings).filter(
          key => allSettings[key].required
        );
        const result = await this.systemManager.applySettings(requiredSettings);
        return result.success
          ? APIResponse.success(result)
          : APIResponse.partial(result, {
              message: 'Some required settings failed to apply'
            });
      }
    );
    ...
    this.api.registerRoute('/system/settings/revert', 'POST', async data => {
      ...
      return result.success
        ? APIResponse.success(result)
        : APIResponse.partial(result, {
            message: 'Some settings failed to revert'
          });
    });

    this.api.registerRoute(
      '/system/settings/revert-all-script',
      'POST',
      async () => {
    ...
    this.api.registerRoute(
      '/system/settings/generate-script',
      'POST',
      async data => {
    ...
    this.api.registerRoute('/system/sip-status', 'GET', async () => {
      if (this.platform !== 'macos') {
        return APIResponse.error({
          message: 'SIP status only available on macOS'
        });
      }
    ...
    this.api.registerRoute('/pm2-processes/list', 'GET', async () => {
      if (this.platform !== 'macos') {
        return APIResponse.error({
          message: 'Launch agents only available on macOS'
        });
      }
    ...
    this.api.registerRoute('/pm2-processes/create', 'POST', async data => {
      ...
      const processOptions = {
        ...
        description:
          options.description ||
          data.description ||
          `Up4evr auto-start for ${path.basename(appPath)}`
      };

      const result = await this.processManager.createAutoStartEntry(
        appPath,
        processOptions
      );
      return result.success
    ...
    this.api.registerRoute('/pm2-processes/create-web', 'POST', async data => {
      if (this.platform !== 'macos') {
        return APIResponse.error({
          message: 'Web application launch agents only available on macOS'
        });
      }
      ...
      const result = await this.processManager.createWebAppPM2Process(
        name,
        url,
        browserPath,
        options
    ...
    this.api.registerRoute('/monitoring/applications', 'GET', async () => {
      const data = this.monitoring.getCurrentData();
      let applications = DataTransformer.sanitizeApplicationList(
        data.applications
      );
    ...
    this.api.registerRoute(
      ...
      async data => {
        ...
        return APIResponse.success({
          message: `Application ${name} added to monitoring`
        });
      }
    );
    ...
    this.api.registerRoute('/monitoring/config', 'GET', async () => {
      const config = {
        ...
        applications: Array.from(
          this.monitoring.watchedApplications.entries()
        ).map(([name, config]) => ({
          name,
          ...config
        }))
    ...
    this.api.registerRoute(
      ...
      async (data, context) => {
        ...
        const currentMonitors =
          this.config.get('monitoring.pingMonitors') || [];
    ...
    this.api.registerRoute(
      ...
      async (data, context) => {
        ...
        return APIResponse.success({
          message: 'Ping monitor removed successfully'
        });
      }
    );

    this.api.registerRoute(
      '/monitoring/ping-monitors/:id/test',
      'POST',
      async (data, context) => {
        const monitorId = context?.params?.id || data.id;
        if (!monitorId) throw new Error('Monitor ID is required');
        ...
        const testResult = await this.testPingConnectivity(
          monitor.ipAddress,
          monitor.timeout
        );
    ...
    this.api.registerRoute('/config-profiles', 'POST', async data => {
      ...
      const result = await this.profiles.saveProfile(
        name,
        description,
        settings,
        options
    ...
    this.api.registerRoute(
      '/config-profiles/:id/export',
      'POST',
      async (data, context) => {
        const profileId = context?.params?.id || data.id;
        if (!profileId) throw new Error('Profile ID is required');
        const result = await this.profiles.exportProfile(
          profileId,
          data.exportPath
        );
        return APIResponse.success(result);
    ...
    this.api.registerRoute('/config-profiles/current', 'POST', async data => {
      ...
      const result = await this.profiles.createProfileFromCurrentConfig(
        name,
        description,
        options
      );
      return APIResponse.success(
        result,
        'Profile created from current configuration'
      );
    });
    ...
    this.api.registerRoute('/health/score', 'GET', async () => {
      ...
      const healthReport = this.healthScoring.generateHealthReport(
        systemData,
        configData
      );
    ...
    this.api.registerRoute('/health/recommendations', 'GET', async () => {
      const systemData = this.monitoring.getCurrentData();
      const configData = this.config.get();
      const healthScore = this.healthScoring.calculateHealthScore(
        systemData,
        configData
      );
      return APIResponse.success({
    ...
    this.api.registerRoute('/service/install', 'POST', async () => {
      ...
      return APIResponse.success({
        ...
        instructions:
          'Please use PM2 or similar process manager for production deployment'
      });
    });
      ...
      if (this.logger) {
        await this.logger.integration(
          ...
          `Slack notification test ${result.success ? 'succeeded' : 'failed'}`,
          {
            ...
            action: result.success
              ? 'notification_test_slack_success'
              : 'notification_test_slack_failure'
          }
        );
      }
    ...
    this.api.registerRoute(
      ...
      async data => {
        ...
        if (this.logger) {
          await this.logger.integration(
            1,
            'Discord notification test initiated',
            {
              hasWebhookUrl: !!data.webhookUrl,
              hasMessage: !!data.message,
        ...
        if (this.logger) {
          await this.logger.integration(
            ...
            `Discord notification test ${result.success ? 'succeeded' : 'failed'}`,
            {
              ...
              message: result.message,
              action: result.success
                ? 'notification_test_discord_success'
                : 'notification_test_discord_failure'
            }
          );
        }
    ...
    this.api.registerRoute(
      ...
      async data => {
        ...
        if (this.logger) {
          await this.logger.integration(
            1,
            'Webhook notification test initiated',
            {
              hasUrl: !!data.url,
              hasMessage: !!data.message,
        ...
        if (this.logger) {
          await this.logger.integration(
            ...
            `Webhook notification test ${result.success ? 'succeeded' : 'failed'}`,
            {
              ...
              message: result.message,
              action: result.success
                ? 'notification_test_webhook_success'
                : 'notification_test_webhook_failure'
            }
          );
        }
      ...
      return APIResponse.success({
        systemChecks: [
          ...
          },
          {
            ...
            status: 'checking',
            description:
              'Checking available disk space for logging and caching',
            required: true
          },
          {
            id: 'permissions',
    ...
    this.api.registerRoute(
      ...
      async () => {
        ...
        return APIResponse.success({
          essentialSettings: systemSettingsData.essentialSettings.map(
            setting => ({
              id: setting.id,
              name: setting.name,
    ...
    this.api.registerRoute(
      ...
      async data => {
        ...
        for (const settingId of selectedSettings) {
          try {
            const result =
              await this.systemManager.applySystemSetting(settingId);
            results.push({
              id: settingId,
              success: result.success,
    ...
    this.api.registerRoute('/setup-wizard/run-tests', 'POST', async data => {
      ...
      return APIResponse.success({
        ...
        overallStatus: validationResults.every(r => r.passed)
          ? 'passed'
          : 'failed',
        criticalFailures: validationResults.filter(r => !r.passed && r.critical)
          .length
      });
    });
      ...
      return APIResponse.success({
        verificationChecks: [
          ...
          },
          {
            ...
            message: systemStatus.settingsCompliant
          ...
          },
          {
            ...
            message: this.monitoring?.isRunning()
              ? 'Monitoring active'
    ...
    this.api.registerRoute('/setup-wizard/log-action', 'POST', async data => {
      ...
      console.log(
        `[SETUP-WIZARD] Action: ${action}, Step: ${step}, Selected: ${selectedCount}/${totalCount}, Time: ${timestamp}`
      );
  ...
  async startMonitoring(interval = null) {
    ...
    if (monitoringConfig.thresholds) {
      const thresholds = {};
      for (const [metric, levels] of Object.entries(
  ...
  getInstallationSettings() {
    ...
    const settings =
      this.config.get('installationSettings') ||
      this.getDefaultInstallationSettings();
    return settings;
  }
  ...
  async testInstallationSettings(settings) {
    ...
    if (
      ...
      Object.keys(settings.customParameters).length > 0
    ) {
      tests.push({
    ...
    if (settings.camera) {
      ...
      if (threshold !== undefined) {
        detailedResults.camera.threshold =
          threshold >= 0 && threshold <= 100
            ? `Valid: ${threshold}%`
            : `Invalid: ${threshold}% (outside 0-100% range)`;
        detailedResults.camera.status =
          threshold >= 0 && threshold <= 100 ? 'success' : 'warning';
      }

      const timeout = settings.camera.timeout;
      if (timeout !== undefined) {
        detailedResults.camera.timeout =
    ...
    if (settings.capacitive) {
      const threshold = settings.capacitive.threshold;
      if (threshold !== undefined) {
        detailedResults.capacitive.threshold =
          threshold >= 0 && threshold <= 100
            ? `Valid: ${threshold}%`
            : `Invalid: ${threshold}% (outside 0-100% range)`;
        detailedResults.capacitive.status =
          threshold >= 0 && threshold <= 100 ? 'success' : 'warning';
      }

      const pins = settings.capacitive.activePins;
      if (pins && Array.isArray(pins)) {
        detailedResults.capacitive.pins =
          pins.length > 0
    ...
    if (settings.audio) {
      const threshold = settings.audio.threshold;
      if (threshold !== undefined) {
        detailedResults.audio.threshold =
          threshold >= 0 && threshold <= 100
            ? `Valid: ${threshold}dB`
            : `Invalid: ${threshold}dB (outside 0-100dB range)`;
        detailedResults.audio.status =
          threshold >= 0 && threshold <= 100 ? 'success' : 'warning';
      }
  ...
  getNotificationConfig() {
    ...
    if (
      existingConfig &&
      (existingConfig.slack || existingConfig.discord || existingConfig.webhook)
    ) {
      return {
  ...
  async saveNotificationConfig(config) {
    try {
      ...
      if (this.monitoring) {
        ...
        console.log(
          '[PLATFORM] Notification config saved and applied to live monitoring service.'
        );
      }
  ...
  async shutdown() {
    ...
    if (this.logger) {
      await this.logger.system(
        LOG_LEVELS.INFO,
        'Platform manager shutting down'
      );
    }
    ...
    if (this.logger) {
      await this.logger.system(
        LOG_LEVELS.INFO,
        'Platform manager shutdown complete'
      );
    }
