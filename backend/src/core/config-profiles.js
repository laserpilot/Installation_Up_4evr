...
class ConfigurationProfiles {
  ...
  async initialize() {
    try {
      ...
      console.warn(
        '[CONFIG-PROFILES] Failed to initialize profiles directory:',
        error.message
      );
    }
  }
  ...
  getBuiltInProfiles() {
    return {
      'gallery-installation': {
        ...
        description:
          'Optimized settings for gallery and museum installations with extended uptime requirements',
      ...
      'interactive-kiosk': {
        ...
        description:
          'Settings for public interactive installations with security and stability focus',
        category: 'kiosk',
        version: '1.0.0',
        builtIn: true,
        settings: {
      ...
      'performance-critical': {
        ...
        description:
          'Maximum performance settings for real-time interactive installations',
      ...
      'development-testing': {
        ...
        description:
          'Flexible settings for development and testing environments',
  ...
  async listProfiles() {
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          try {
            ...
            console.warn(
              `[CONFIG-PROFILES] Failed to load profile ${file}:`,
              error.message
            );
          }
        }
      ...
      console.warn(
        '[CONFIG-PROFILES] Failed to read profiles directory:',
        error.message
      );
    }
  ...
  async exportProfile(profileId, exportPath = null) {
    ...
    console.log(
      `[CONFIG-PROFILES] Profile exported: ${profile.name} -> ${targetPath}`
    );
  ...
  async importProfile(importPath) {
    ...
    console.log(
      `[CONFIG-PROFILES] Profile imported: ${profile.name} (${profileId})`
    );
  ...
  async applyProfileSettings(settings) {
    ...
    console.log(
      `[CONFIG-PROFILES] Applied settings for: ${updates.join(', ')}`
    );
  ...
  async createProfileFromCurrentConfig(name, description, options = {}) {
    ...
    if (settings.monitoring?.applications) {
      settings.monitoring.applications =
        settings.monitoring.applications.filter(app => !app.systemGenerated);
    }
    ...
    if (settings.monitoring?.thresholds) {
      Object.keys(settings.monitoring.thresholds).forEach(metric => {
        ...
        if (thresholds.warning && thresholds.critical) {
          if (thresholds.warning >= thresholds.critical) {
            errors.push(
              `${metric}: Warning threshold must be less than critical threshold`
            );
          }
        }
      });
