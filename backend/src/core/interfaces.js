...
class PlatformFactory {
  ...
  static createSystemManager() {
    ...
    try {
      ...
      throw new Error(
        `Failed to load system manager for ${platform}: ${error.message}`
      );
    }
  }
  ...
  static createProcessManager() {
    ...
    try {
      ...
      throw new Error(
        `Failed to load process manager for ${platform}: ${error.message}`
      );
    }
  }
  ...
  static createMonitoringProvider() {
    const platform = this.getPlatform();
    try {
      const MonitoringProvider = require(
        `../platform/${platform}/monitoring-provider`
      );
      return new MonitoringProvider();
    } catch (error) {
      throw new Error(
        `Failed to load monitoring provider for ${platform}: ${error.message}`
      );
    }
  }
}
