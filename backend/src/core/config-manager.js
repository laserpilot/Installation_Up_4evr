...
class ConfigManager {
  ...
  async createMasterProfile(name, description, options = {}) {
    const installationId =
      options.installationId || this.generateInstallationId();
    const masterConfig = {
      installationId,
  ...
  async addLaunchAgent(agentInfo) {
    ...
    if (
      this.get('monitoring.watchLaunchAgents') &&
      this.get('monitoring.autoAddNewAgents')
    ) {
  ...
  async removeLaunchAgent(agentId) {
    ...
    const filteredApps = monitoredApps.filter(
      app =>
        !(
          app.type === 'launch-agent' &&
          app.autoGenerated &&
          app.path === agentId
        )
    );
  ...
  async exportConfig(filePath = null) {
    const exportPath =
      filePath || path.join(this.configDir, `config-backup-${Date.now()}.json`);
    const configData = JSON.stringify(this.config, null, 2);
  ...
  async exportMasterProfile(filePath = null) {
    ...
    const exportPath =
      ...
      path.join(
        this.configDir,
        `master-profile-${master.installationId || Date.now()}.json`
      );
