...
class ValidationWorkflow {
  ...
  setupValidationTests() {
    return [
      ...
      },
      {
        ...
        priority: 'critical',
        description:
          'Check CPU, memory, and disk usage are within acceptable ranges',
      ...
      // Configuration Tests
      {
        ...
        priority: 'high',
        description:
          'Verify system preferences are set correctly for installation',
      ...
      },
      {
        ...
        priority: 'high',
        description:
          'Check that launch agents are properly installed and active',
      ...
      // Security Tests
      {
        ...
        priority: 'medium',
        description:
          'Verify security settings are appropriate for installation',
  ...
  async runFullValidation(options = {}) {
    ...
    try {
      ...
      console.log(
        `[VALIDATION] Starting validation workflow with ${selectedTests.length} tests`
      );
      ...
      console.log(
        `[VALIDATION] Completed in ${duration}ms. ${summary.passed}/${summary.total} tests passed`
      );
  ...
  async testSystemUptime() {
    ...
    if (uptimeHours >= 1) {
      ...
      return {
        ...
        details: { uptimeHours, uptimeSeconds },
        recommendations: [
          'Allow system to run for at least 1 hour to ensure stability'
        ]
      };
    }
  }
    ...
    if (displayArray.length === 0) {
      return {
        ...
        recommendations: [
          ...
          'Verify graphics card functionality'
        ]
      };
    }
  ...
  async testNetworkConnectivity() {
    ...
    const hasActiveInterface = details.interfaces.some(
      iface =>
        iface.status === 'active' &&
        iface.ipAddress &&
  ...
  async testCriticalApplications() {
    ...
    const runningCriticalApps = criticalApps.filter(
      app => app.status === 'running'
    );
    const stoppedCriticalApps = criticalApps.filter(
      app => app.status !== 'running'
    );
  ...
  async testLaunchAgents() {
    ...
    return {
      ...
      details: {},
      warning:
        'Launch agent validation requires integration with launch agents module'
    };
  }
  ...
  async testSecuritySettings() {
    ...
    return {
      ...
      details,
      warning:
        'Comprehensive security testing requires additional system integration'
    };
  }
