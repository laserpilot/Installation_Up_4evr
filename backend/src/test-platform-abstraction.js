...
class PlatformAbstractionTests {
  ...
  async testInterfaces() {
    ...
    await this.test(
      'SystemManagerInterface prevents direct instantiation',
      async () => {
        try {
          ...
          assert(
            error.message.includes('Cannot instantiate abstract interface')
          );
        }
      }
    );
    ...
    await this.test(
      'ProcessManagerInterface prevents direct instantiation',
      async () => {
        try {
    ...
    await this.test(
      'PlatformFactory creates appropriate managers',
      async () => {
  ...
  async testPlatformFactory() {
    ...
    await this.test(
      'Factory throws error for unsupported platform',
      async () => {
  ...
  async testConfigManager() {
    ...
    await this.test(
      'ConfigManager initializes with default config',
      async () => {
        ...
        assert(
          ...
          'Config should have notifications section'
        );
      }
    );
    ...
    await this.test('ConfigManager supports dot notation access', async () => {
      ...
      assert(
        ...
        'Monitoring interval should be a number'
      );
    ...
    await this.test(
      'ConfigManager persists configuration changes',
      async () => {
        const configManager = new ConfigManager();
        await configManager.initialize();
  ...
  async testAPIManager() {
    ...
    await this.test('APIManager registers and handles routes', async () => {
      ...
      const result = await apiManager.handleRequest('/test', 'GET', {
        input: 'test'
      });
  ...
  async testMonitoringCore() {
    ...
    await this.test('MonitoringCore manages watched applications', async () => {
      ...
      monitoring.addApplication('TestApp', '/path/to/app', {
  ...
  async testPlatformManager() {
    ...
    await this.test(
      'PlatformManager provides feature information',
      async () => {
        const platformManager = new PlatformManager();
        await platformManager.initialize();
  ...
  async testCompatibilityLayer() {
    ...
    await this.test(
      'CompatibilityLayer initializes in platform mode',
      async () => {
        ...
        if (compatibility.legacyMode) {
          console.log(
            '    Note: Platform manager fallback to legacy mode (acceptable in test environment)'
          );
          assert(compatibility.isUsingPlatformManager() === false);
        } else {
          assert(
            compatibility.legacyMode === false,
            'Should not be in legacy mode'
          );
          assert(
            compatibility.isUsingPlatformManager() === true,
            'Should be using platform manager'
          );
        }
      }
    );
    ...
    await this.test(
      'CompatibilityLayer falls back to legacy mode',
      async () => {
        const compatibility = new CompatibilityLayer();
  ...
  async testMacOSImplementations() {
    ...
    await this.test(
      'MacOS ProcessManager gets running applications',
      async () => {
        const processManager = PlatformFactory.createProcessManager();
    ...
    await this.test(
      'MacOS MonitoringProvider gets system metrics',
      async () => {
