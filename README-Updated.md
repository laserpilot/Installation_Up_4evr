...
<!-- 🔄 UPDATE NOTE: Add 2025 changelog from original here, but modernize the language -->
...
<!-- 🔄 UPDATE NOTE: This changelog needs to be updated to reflect actual 2025 changes, not just plans -->

## Table of Contents

<!-- ⚠️ BROKEN LINKS: All these anchor links need to be fixed -->
...
<!-- 🔄 UPDATE NOTE: Update this preface to reflect 2025 status and modernization -->
...
<!-- ✅ GOOD: This intro is still relevant, keep it -->
...
<!-- ⚠️ SECURITY WARNING: This disclaimer needs to be much stronger for 2025 -->

## Prepare your software

<!-- ✅ GOOD: This section is timeless, keep it -->
...
<!-- 🔄 UPDATE NOTE: This entire section needs major modernization -->
<!-- 🔄 UPDATE NOTE: Add "Configuration as Code" introduction here -->
<!-- 🔄 UPDATE NOTE: Provide a shell script that does all these settings automatically -->
...
<!-- 📸 UPDATE IMAGES: All screenshots need updating to current macOS version -->

- ##### Screensaver

	Disable your screensaver. Set it's time to **Never** 
	
	<!-- 🔄 UPDATE: Add command-line equivalent: `defaults -currentHost write com.apple.screensaver idleTime 0` -->
...
- ##### Desktop
	...
	<!-- 🔄 UPDATE: Add command-line equivalent for setting desktop background -->
...
- ##### Energy Saver
    ...
    - Restart automatically after computer power failure or kernel panic is enabled by default as of 10.8. You can view the current status of both of these settings in the terminal with the following command: `sudo systemsetup -getrestartfreeze -getrestartpowerfailure`
     ...
     <!-- 🔄 UPDATE: This is exactly what we should do! Create that script -->
     
 ![Power_settings](images/PowerSettings.png)
 
 <!-- 📸 UPDATE IMAGES: Update screenshot -->

- ##### Security
    These tips in particular open up a range of vulnerabilities. Proceed with caution.
    
    <!-- ⚠️ SECURITY WARNING: This entire section needs major security updates -->
    ...
    <!-- ⚠️ REMOVE: This is terrible security advice for 2025 -->
    <!-- 🔄 UPDATE: Replace with app signing and notarization guidance -->
...
- ##### Users and Groups
    ...
    **IMPORTANT:** If you have any security concerns _at all_ do  not automatically login to an admin user. Create a new standard user and use this setting to login to that account. Using a standard user may make certain sudo commands more difficult though.
    ...
    <!-- ⚠️ SECURITY WARNING: Strengthen this guidance - emphasize standard user approach -->
...
- ##### Software Update
    ...
    <!-- 🔄 UPDATE: Add command-line equivalent: `sudo softwareupdate --schedule off` -->
      

 ![Update_disable](images/Auto_update_disable1.png)
  ![Update_disable](images/Auto_update_disable2.png)
  
  <!-- 📸 UPDATE IMAGES: Update screenshots -->
...
- ##### Sharing
    ...
    <!-- 🔄 UPDATE: Add command-line equivalents for enabling these services -->
 ...
 - ##### Network
    Your computer should almost always be hardwired to the network with ethernet - do not trust wifi :)
     ...
     <!-- 🔄 UPDATE: Add command-line methods for WiFi management -->
 ...
 - ##### Bluetooth
    ...
    <!-- 🔄 UPDATE: Add command-line equivalent: `defaults write /Library/Preferences/com.apple.Bluetooth BluetoothAutoSeekKeyboard -bool false` -->
...
<!-- ⚠️ REMOVE: Do not recommend disabling SIP -->
<!-- 🔄 UPDATE: Replace with Focus Modes programmatic control -->
...
<!-- ⚠️ REMOVE: Do not recommend disabling SIP or modifying system files -->
<!-- 🔄 UPDATE: Find SIP-compatible alternatives -->
...
<!-- ⚠️ MAJOR UPDATE: This entire section should discourage SIP disabling -->
<!-- 🔄 UPDATE: Provide SIP-compatible alternatives for every recommendation -->
...
<!-- ⚠️ REMOVE: Do not include SIP disabling instructions -->
...
<!-- 🔄 UPDATE: Replace Tinkertool with `defaults write` command examples -->
...
<!-- ⚠️ REMOVE: Do not recommend modifying system files -->
...
<!-- 🔄 UPDATE: Verify if App Nap is still relevant in 2025 -->
...
<!-- 🔄 UPDATE: Emphasize the Launch Agent approach over Login Items -->
...
<!-- 📸 UPDATE IMAGES: Update screenshot -->
<!-- 🔄 UPDATE: De-emphasize this approach in favor of Launch Agents -->
...
<!-- 🔄 UPDATE: Consider removing Automator recommendation entirely -->
...
<!-- 📸 UPDATE IMAGES: Update screenshot or remove -->
...
<!-- ✅ GOOD: This section has good fundamentals, but needs updating -->
...
<!-- 🔄 UPDATE: De-emphasize cronjob approach, focus on Launch Agents -->
...
#### Process Management: LaunchAgents vs PM2

<!-- 🔄 NEW SECTION: Comparing native vs third-party solutions -->

Before diving into the technical details, it's worth understanding your two main options for keeping applications running:

**LaunchAgents (Native macOS Solution):**
- ✅ **No installation required** - Built into every Mac
- ✅ **Works with any application** - Not just Node.js apps
- ✅ **Native macOS integration** - Handles permissions, user sessions, and boot sequences properly
- ✅ **System-level reliability** - Managed by the OS itself
- ❌ **More complex setup** - Requires creating XML plist files
- ❌ **Limited monitoring** - Basic process management only
- ❌ **Learning curve** - Less familiar to web developers

**PM2 (Third-party Node.js Process Manager):**
- ✅ **Simple commands** - `pm2 start`, `pm2 restart`, `pm2 monitor`
- ✅ **Excellent for Node.js apps** - Built-in support for JS/TS applications
- ✅ **Rich monitoring** - Built-in logs, CPU/memory monitoring, web dashboard
- ✅ **Developer-friendly** - Familiar to web developers
- ✅ **Easy clustering** - Run multiple instances of your app
- ❌ **Requires installation** - Need Node.js + npm install steps
- ❌ **Additional complexity** - Another service to manage
- ❌ **Boot sequence setup** - Still need LaunchAgent to start PM2 on boot

**When to choose each:**

**Use LaunchAgents if:**
- You want zero dependencies and maximum reliability
- Your app is not Node.js based (native apps, Python, etc.)
- You prioritize system integration over convenience
- You have a simple single-process application
- You want the most "Mac-native" approach

**Use PM2 if:**
- Your installation application is Node.js based
- You need advanced monitoring and logging capabilities
- You're more comfortable with npm/Node.js tooling
- You need to manage multiple processes or clustering
- You want a web-based monitoring dashboard

**Hybrid approach:**
Many developers use PM2 to manage their Node.js application, but still create a LaunchAgent to automatically start PM2 on boot. This gives you the best of both worlds - PM2's rich features for your app, with LaunchAgent ensuring PM2 itself starts reliably.

<!-- 🔄 UPDATE: Consider adding a "Quick Start" command comparison here -->

Example PM2 setup for installations:
```bash
# Install PM2 globally
npm install -g pm2

# Start your app with PM2
pm2 start app.js --name "installation-app"

# Configure PM2 to restart on boot (creates LaunchAgent automatically)
pm2 startup
pm2 save
```
...
<!-- ✅ GOOD: This section is still accurate -->
...
<!-- ✅ GOOD: This section is excellent, keep it -->
...
<!-- 🔄 UPDATE: De-emphasize third-party GUI apps -->
...
<!-- 🔄 UPDATE: Add note about preferring manual creation for repeatability -->

#### Unusual Cases

<!-- ✅ GOOD: Keep this section but update examples -->
...
<!-- 🔄 UPDATE: De-emphasize this approach - Launch Agent KeepAlive is superior -->
...
<!-- 🔄 UPDATE: Remove cronjob reference, focus on Launch Agents -->
...
<!-- ✅ GOOD: Keep this but add warnings -->

## Reboot periodically

<!-- 🔄 UPDATE: Remove Onyx recommendation, provide scripted solution -->
...
<!-- 🔄 UPDATE: Replace with Launch Agent that runs `sudo shutdown -r now` -->
...
<!-- 📸 UPDATE IMAGES: This feature no longer exists, remove screenshot -->
<!-- ⚠️ REMOVE: This GUI no longer exists -->
...
<!-- 🔄 UPDATE: Some of these may be outdated, verify current options -->
...
<!-- 🔄 UPDATE: Consider modern cloud storage options -->
...
<!-- 🔄 UPDATE: Modernize logging approaches -->
...
<!-- 🔄 UPDATE: Emphasize structured logging (JSON) -->
...
<!-- 🔄 UPDATE: While still functional, consider more robust monitoring approaches -->
...
<!-- 🔄 UPDATE: Remove Lingon reference, focus on manual launchd -->
...
<!-- ✅ GOOD: These examples are still relevant -->
...
<!-- 🔄 UPDATE: Improve security guidance for webhooks -->
...
<!-- ⚠️ SECURITY WARNING: This needs proper security guidance -->
...
<!-- 🔄 UPDATE: Add security best practices for webhook URLs -->

### Uncategorized and Out of Date Tips

<!-- 🗑️ MAJOR CLEANUP: This entire section needs heavy editing -->
<!-- 🔄 UPDATE: Remove most of this section, keep only relevant parts -->
...
<!-- 🔄 UPDATE: Add modern backup solutions -->
...
<!-- ✅ GOOD: This is still useful, keep it -->
...
<!-- 🗑️ REMOVE: Author already recommends against this -->
...
<!-- 🗑️ REMOVE: Author already marked as outdated -->
...
[REST OF EMAIL SECTION CONTINUES...]

<!-- 🗑️ REMOVE: Entire email section - already marked outdated by author -->

#### Cronjob process keepalive
This method is sort of deprecated in relation to the launchd method - you can run shell scripts with Lingon and launchd in the same manner as what we've got here.

<!-- 🗑️ REMOVE: Author calls this deprecated -->

[REST OF CRONJOB SECTION CONTINUES...]
...
<!-- 🗑️ REMOVE: Likely outdated and very specific -->

## Alternate resources:

<!-- ✅ GOOD: Keep this section but update links -->
...
<!-- 🔄 UPDATE: Verify these links still work -->
...
## 🔧 SUMMARY OF MAJOR UPDATES NEEDED:

### HIGH PRIORITY (Security & Compatibility):
- ⚠️ Remove ALL System Integrity Protection (SIP) disabling recommendations
- 🔄 Replace GUI instructions with command-line equivalents
- ⚠️ Strengthen security warnings throughout
- 🔄 Add Apple Silicon specific guidance
- 📸 Update ALL screenshots to current macOS

### MEDIUM PRIORITY (Modernization):
- 🔄 Create "Configuration as Code" setup script
- 🔄 Replace third-party GUI tools with scriptable solutions
- 🔄 Improve Slack webhook security guidance
- 🔄 Add structured logging recommendations

### LOW PRIORITY (Cleanup):
- 🗑️ Remove ~40% of outdated "Uncategorized" section
- 🔄 Fix all broken table of contents links
